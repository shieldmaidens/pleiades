...
impl DiskStorage {
    ...
    pub fn get(&self, key: MetaKeyValuePair) -> Result<KeyValuePair, StorageError> {
        let (kvp, cf_handle) = self.key_unwinder(&key)?;
    ...
    fn key_unwinder(&self, key: &MetaKeyValuePair) -> Result<(KeyValuePair, Arc<BoundColumnFamily>), StorageError> {
        if key.shard == 0 {
        ...
        let cf_handle = match self.db.cf_handle(&key.shard.to_string()) {
            None => match self.create_cf(key.shard) {
                Ok(v) => v,
...
mod test {
    ...
    #[test]
    #[serial]
    fn put_and_get() -> Result<(), StorageError> {
