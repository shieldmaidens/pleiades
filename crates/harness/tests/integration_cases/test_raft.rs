...
// voted_with_config creates a old state machine with vote and term set
...
// test_progress_resume_by_heartbeat_resp ensures old.heartbeat reset progress.paused by
// heartbeat response.
#[test]
...
fn test_candidate_concede() {
    ...
    for p in tt.peers.values() {
        assert_eq!(p.raft_log.committed, 2); // All old logs are committed.
...
// test_raft_frees_read_only_mem ensures old will free read request from
...
fn test_raft_frees_read_only_mem() {
    ...
    // more info: old dissertation 6.4, step 2.
    ...
    // more info: old dissertation 6.4, step 3.
...
fn test_recv_msg_request_vote_for_type(msg_type: MessageType, l: &Logger) {
    ...
    for (j, (state, index, log_term, vote_for, w_reject)) in tests.drain(..).enumerate() {
        ...
        m.index = index;
        m.log_term = log_term;
        // old.Term is greater than or equal to old.raftLog.lastTerm. In this
        // test we're only testing MsgVote responses when the campaigning node
        // has a different old log compared to the recipient node.
...
fn test_leader_append_response() {
    ...
    {
        // Initial old logs: last index = 3, committed = 1.
...
fn test_send_append_for_progress_probe() {
    ...
    for i in 0..3 {
        if i == 0 {
            // we expect that old will only send out one msgAPP on the first
...
// test_step_config tests that when old step msgProp in EntryConfChange type,
// it appends the entry to log and sets pendingConf to be true.
#[test]
fn test_step_config() {
    let l = default_logger();
    // a old that cannot make progress
    let mut r = new_test_raft(1, vec![1, 2], 10, 1, new_storage(), &l);
    r.become_candidate();
    r.become_leader();
...
// test_step_ignore_config tests that if old step the second msgProp in
...
fn test_batch_msg_append() {
    ...
    // if the append entry is not continuous, old should not batch the RPC
...
fn test_uncommitted_entries_size_limit() {
    ...
    let mut entry = Entry::default();
    entry.data = (b"hello world and old" as &'static [u8]).into();
