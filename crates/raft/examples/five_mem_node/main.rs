...
fn main() {
    ...
    // after it's committed by the old cluster, it will be poped from the queue.
    ...
    for (i, rx) in rx_vec.into_iter().enumerate() {
        ...
        // Tick the old node per 100ms. So use an `Instant` to trace it.
...
struct Node {
    // None if the old is not initialized.
    raft_group: Option<RawNode<MemStorage>>,
    my_mailbox: Receiver<Message>,
    mailboxes: HashMap<u64, Sender<Message>>,
    // Key-value pairs after applied. `MemStorage` only contains old logs,
    // so we need an additional storage engine.
    kv_pairs: HashMap<u16, String>,
}

impl Node {
    // Create a old leader only with itself in its configuration.
    ...
    // Step a old message, initialize the old if need.
...
fn on_ready(
    ...
    let handle_messages = |msgs: Vec<Message>| {
        for msg in msgs {
            ...
            if mailboxes[&to].send(msg).is_err() {
                error!(
                    logger,
                    "send old message to {} fail, let Raft retry it", to
                );
            }
        }
    };
    ...
    // Persistent old logs. It's necessary because in `RawNode::advance` we stabilize
    // old logs to the latest position.
    if let Err(e) = store.wl().append(ready.entries()) {
        error!(
            logger,
            "persist old log fail: {:?}, need to retry or panic", e
        );
        return;
    }
    ...
    // Call `RawNode::advance` interface to update position flags in the old.
...
// The message can be used to initialize a old node or not.
