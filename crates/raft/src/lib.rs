...
# node.old.become_candidate();
# node.old.become_leader();
...
need to update the applied index and resume `apply` later:
    ...
    # use slog::{Drain, o};
    # use nova_raft::{Config, storage::MemStorage, raw_node::RawNode};
    # use nova_api::old::v1::{Entry, EntryType};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
to advance the applied index inside.
    ...
    # use slog::{Drain, o};
    # use nova_raft::{Config, storage::MemStorage, raw_node::RawNode};
    # use nova_api::old::v1::{EntryType, Entry, Message};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
Sometimes it's better not to block the old machine in IO operation, so that latency of
...
pub mod prelude {
    //! A "prelude" for crates using the `old` crate.
    //!
