...
//! The raw node of the old module.
//!
//! This module contains the value types for the node and it's connection to other
//! nodes but not the old consensus itself. Generally, you'll interact with the
...
impl Ready {
    ...
    /// If it contains a MsgSnap message, the application MUST report back to old
...
impl<T: Storage> RawNode<T> {
    ...
    /// Propose proposes data be appended to the old log.
    ...
    fn gen_light_ready(&mut self) -> LightReady {
        ...
        rd.committed_entries = raft
            ...
            .unwrap_or_default();
        // Update old uncommitted entries size
    ...
    pub fn ready(&mut self) -> Ready {
        ...
        // For more details, check old thesis 10.2.1.
    ...
    /// get a reasonable batch size. However, it's still required that the updates can be read by old from the
    ...
    /// Grabs the snapshot from the old if available.
    #[inline]
