...
pub enum Error {
    ...
    #[error("old: cannot step old local message")]
    StepLocalMsg,
    /// The old peer is not found and thus cannot step.
    #[error("old: cannot step as peer not found")]
    StepPeerNotFound,
    /// The proposal of changes was dropped.
    #[error("old: proposal dropped")]
    ProposalDropped,
    ...
    #[error("old: request snapshot dropped")]
...
/// A result type that wraps up the old errors.
