...
/// A constant represents invalid id of old.
pub const INVALID_ID: u64 = 0;
/// A constant represents invalid index of old log.
...
/// The core struct of old consensus.
///
...
pub struct RaftCore<T: Storage> {
    ...
    /// Which peer this old is voting for.
    pub vote: u64,
    ...
    ///
    /// If this is Some(id), we follow the procedure defined in old thesis 3.10.
    ...
    // when old changes its state to follower or candidate.
    randomized_election_timeout: usize,
    min_election_timeout: usize,
    max_election_timeout: usize,

    /// The logger for the old structure.
...
/// A struct that represents the old consensus itself. Stores details concerning the current
...
impl<T: Storage> Raft<T> {
    /// Creates a new old for use on the node.
    ...
    /// Creates a new old for use on the node with the default logger.
    ///
    ...
    /// Returns whether the current old is in lease.
    ...
    /// it every time a old state machine is initialized or a snapshot is applied.
    ...
    /// Checks whether the old group is using group commit and consistent
    /// over group.
    ///
    ...
    ///
    /// The check is useful usually when old is leader.
...
impl<T: Storage> Raft<T> {
    ...
    /// Notifies that these old logs have been persisted.
    ...
    /// Steps the old along via a message. This should be called everytime your old receives a
    /// message from a peer.
    ...
    fn has_unapplied_conf_changes(&self, lo: u64, hi: u64, context: GetEntriesContext) -> bool {
        ...
        // when old sends entries via the Ready struct for application.
        // TODO(pavelkalinnikov): find a way to budget memory/bandwidth for this scan
        // outside the old package.
    ...
    // the view of the local old state machine. Otherwise, it returns
    // false.
    ...
    /// For old with no limit(or non-leader old), it always return true.
    #[inline]
    pub fn maybe_increase_uncommitted_size(&mut self, ents: &[Entry]) -> bool {
