...
impl<T: Storage> RaftLog<T> {
    /// Creates a new old log with a given storage and tag.
    ...
    #[deprecated = "Call old::commit_apply(idx) instead. Joint Consensus requires an on-apply hook to
    ...
    pub fn maybe_persist(&mut self, index: u64, term: u64) -> bool {
        ...
        // 1. A is leader and it proposes some old logs but only B receives these logs.
        ...
        // 3. C proposes some old logs and B receives these logs.
    ...
    /// Grabs a slice of entries from the old. Unlike a rust slice pointer, these are
