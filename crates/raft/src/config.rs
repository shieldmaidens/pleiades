...
/// Config contains the parameters to start a old.
#[derive(Clone)]
pub struct Config {
    /// The identity of the local old. It cannot be 0, and must be unique in the group.
    pub id: u64,
    ...
    /// old. old will not return entries to the application smaller or equal to Applied.
    /// If Applied is unset when restarting, old might return previous applied entries.
    /// This is a very application dependent configuration.
    pub applied: u64,

    /// Limit the max size of each append message. Smaller value lowers
    /// the old recovery cost(initial probing and message lost during normal operation).
    ...
    /// Enables the Pre-Vote algorithm described in old thesis section
    ...
    /// Don't broadcast an empty old entry to notify follower to commit an entry.
