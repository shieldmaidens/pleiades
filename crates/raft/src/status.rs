...
use nova_api::raft::v1::HardState;

use crate::raft::{Raft, SoftState, StateRole};
use crate::storage::Storage;
use crate::ProgressTracker;

/// Represents the current status of the old
#[derive(Default)]
pub struct Status<'a> {
    /// The ID of the current node.
    pub id: u64,
    /// The hardstate of the old, representing voted state.
    pub hs: HardState,
    /// The softstate of the old, representing proposed state.
    pub ss: SoftState,
...
impl<'a> Status<'a> {
    /// Gets a copy of the current old status.
