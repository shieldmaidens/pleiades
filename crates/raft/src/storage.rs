...
impl GetEntriesContext {
    /// Used for callers out of old. Caller can customize if it supports async.
...
pub(crate) enum GetEntriesFor {
    ...
    CommitByVote,
    // It's not called by the old itself
    Empty(bool),
}
...
///
/// If any Storage method returns an error, the old instance will
...
pub trait Storage {
    ...
    /// so old state machine could know that Storage needs some time to prepare
...
pub struct MemStorageCore {
    ...
    // entries[i] has old log position i+snapshot.get_metadata().index
    entries: Vec<Entry>,
...
impl MemStorageCore {
    ...
    /// # Panics
    ///
    /// Panics if there is no such entry in old logs.
    ...
    pub fn compact(&mut self, compact_index: u64) -> Result<()> {
        ...
        if compact_index > self.last_index() + 1 {
            panic!(
                "compact not received old logs: {}, last index: {}",
                compact_index,
                self.last_index()
            );
        }
    ...
    pub fn append(&mut self, ents: &[Entry]) -> Result<()> {
        ...
        if self.first_index() > ents[0].index {
            panic!(
                "overwrite compacted old logs, compacted: {}, append: {}",
                self.first_index() - 1,
                ents[0].index,
            );
        }
        if self.last_index() + 1 < ents[0].index {
            panic!(
                "old logs should be continuous, last index: {}, new appended: {}",
                self.last_index(),
                ents[0].index,
            );
        }
...
///
/// A real `Storage` should save both old logs and applied data. However `MemStorage` only
/// contains old logs. So you can call `MemStorage::append` to persist new received unstable old
...
impl MemStorage {
        ...
        ConfState: From<T>,
    {
        ...
        // Setting initial state is very important to build a correct old, as old algorithm
...
mod test {
    ...
    fn test_storage_append() {
        ...
        let mut tests = vec![
            ...
            // overwrite compacted old logs is not allowed
            (
